...
function Header({showhRooms, setShowSearchRooms}) {
  ...
  const queryFnSearch = async (address, checkInDate, checkOutDate, guestNum) => {
  ...
  return (
    ...
    <StHeaderWrapper onClick={handleModalClick}>
      ...
      <button type="submit" onSubmit={handleSearchSubmit}>조회하기</button>
      {/* 데이터를 이용하여 UI 구성 */}
    </form>
    </div>
        <Row>
          ...
          <LanguageIcon/>
          <div onClick={() => setIsOpenModal(true)}>{isOpenModal && <LoginModal setIsOpenModal={setIsOpenModal} />}</div>
        </Row>
